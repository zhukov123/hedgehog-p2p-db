...
## 2026-07-20 - evaluate recovery gate workflow

- Branch: `agent/evaluate-recovery-gate-2026-07-20`
- Task: implement the SQLite metadata `evaluate_recovery_gate` workflow so authority recovery checks persist a durable gate snapshot, audit the decision, and move the evaluated node between `recovering` and `normal`.
- Tests:
...
- Container coverage: Docker is installed, but this task exercised the available in-process multi-node runtime harness instead of the observability compose stack because the change is a metadata workflow and not a container deployment change.
- Severus: direct `sessions_send agentId: severus` failed with `agent not found`; handoff was sent to Discord `#agentchat` parent message `1528871835339591800`, auto-thread `1528871835339591800`. Severus advised making each evaluation immutable with one timestamp and one captured probe snapshot; the workflow persists a single request snapshot per idempotency key.
- PR: https://github.com/zhukov123/hedgehog-p2p-db/pull/26
- Next candidate task: implement the remaining SQLite `accept_invite` workflow or reconcile the open duplicate revocation/recovery-gate PRs before merging overlapping branches.
