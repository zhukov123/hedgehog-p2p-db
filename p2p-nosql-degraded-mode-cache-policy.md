...
This pass defines what head nodes may do when PostgreSQL is unavailable, failing over, restoring, or too stale to answer safely.

Assumptions:
- PostgreSQL is the v1 durable authority for identity, tenant and dataset policy, placement, object visibility, leases, revocation, invitations, reservations, repair, audit, and outbox.
...
- No operation may create new authority while PostgreSQL is unavailable.
- Cached data may only serve client-visible reads for object versions that PostgreSQL already marked committed before the outage.
...
| --- | --- | --- |
| `normal` | PostgreSQL reachable, migrations current, authority cache within age limits | All authorized workflows may run through PostgreSQL transactions. |
| `degraded_read_only` | PostgreSQL unavailable or failover in progress, but selected committed read cache entries are fresh | Serve only allowed cached reads; reject writes, deletes, repair ownership changes, admin mutations, invites, registration, and capacity-affecting workflows. |
| `authority_stale` | PostgreSQL unavailable and required authority cache is too old or missing | Reject client and admin operations except health/status endpoints. Keep agent sessions alive for telemetry buffering only. |
| `recovering` | PostgreSQL reachable again, but cache, outbox, audit, and invariant checks have not caught up | Reject unsafe operations until recovery gates pass. Reads may resume only after committed visibility cache is refreshed. |

Transition rules:
- `normal -> degraded_read_only` when PostgreSQL health fails but read cache age remains inside the policy table below.
- `degraded_read_only -> authority_stale` when any record required for a requested read exceeds its maximum age.
- `degraded_read_only -> recovering` when PostgreSQL returns but cache reload and replay have not completed.
- `recovering -> normal` only after migrations are current, invariant checks pass, outbox lag is within threshold, audit append works, and all authority caches are rebuilt from PostgreSQL.

## Cache Policy Table

| Record | Max cache age | Allowed during PostgreSQL outage | Fail behavior | Revocation rule | Recovery/audit requirement |
| --- | --- | --- | --- | --- | --- |
...
| Revocation epochs | 15 seconds | Only as a deny source, never as an allow source | Fail closed if missing, stale, or lower than request envelope epoch | Highest known revocation wins; cached revocation blocks immediately; absence cannot prove validity | Rebuild revocation cache from PostgreSQL before accepting any signed envelope that depends on it |
...
| Routing hints | 30 seconds | Use only after tenant, dataset, object visibility, placement, and node status checks also pass | Fail closed to `metadata_unavailable`, not best-effort routing | Hints pointing at revoked/quarantined heads or nodes are discarded | Rebuild hints from PostgreSQL/outbox after recovery; do not audit as authority |
...
| Write reservations and leases | 0 seconds | No | Fail closed for create, stream-start, replica completion, commit, expiry conversion, or cleanup release | Any revocation blocks command dispatch immediately; stale completions remain local evidence only | Reconcile partial writes through PostgreSQL workflow; never make object visible from cache |
...
Rejected in every PostgreSQL outage state:
...
- any operation requiring audit and outbox rows in the same PostgreSQL transaction

## Head Cache Implementation Rules

1. Cache entries must include `metadata_revision`, `loaded_at`, `source_txid` or equivalent PostgreSQL revision marker, `policy_version`, and relevant revocation/authority epochs.
...
5. Deny records outlive allow records until PostgreSQL refresh proves otherwise.
6. Any stale record required by a read makes the whole read fail closed.
7. Mutating workflows call `metadata-pg` directly and do not accept cache-backed decisions.
...
11. Restrictive states are sticky during outage: a cached suspended tenant, frozen dataset, revoked actor, quarantined node, delete marker, or expired invitation can deny work, but cannot be cleared without PostgreSQL.

## Required Test Fixtures

Add these before implementing head-node metadata caches:
- PostgreSQL outage rejects write intent, delete, admin mutation, invite accept, repair lease, replica completion, and capacity admission.
...
- In-flight repair copy during outage cannot mark a replica healthy until PostgreSQL fencing workflow accepts it after recovery.
...
- Repair and write completions can preserve local evidence during outage, but PostgreSQL must later decide their semantic result.
...
- invitations, admin mutations, repair leases, capacity admission, and write visibility changes require PostgreSQL
